...
@registry.register_model("dual_branch_rwkv_rwkv")
class Dual_Branch_RWKV_RWKV(Blip2Base):
    """
    ...
    def __init__(
        ...
        self.opt_proj_RWKV_0 = nn.Linear(
            self.q_former_hidden_size, self.opt_model.config.hidden_size
        )
        ...
        if frozen_opt_proj:
            ...
            for name, param in self.opt_proj_RWKV_0.named_parameters():
                param.requires_grad = False
            logging.info('OPT_proj_rwkv_0 is  frozen')
        else:
            for name, param in self.opt_proj_RWKV.named_parameters():
            ...
            for name, param in self.opt_proj_RWKV_0.named_parameters():
                param.requires_grad = True
            logging.info('OPT_proj_rwkv_2 is not frozen')
        ...
        self.rwkv_0 = rwkv_init()
        self.RWKV_block_0 = nn.Sequential(OrderedDict([
            ('rwkv', self.rwkv_0),
            ('opt_proj_RWKV', self.opt_proj_RWKV_0)
        ]))
    ...
    def encode_video_rwkv_0(self, q_hidden_state):
        with self.maybe_autocast():
            ...
            # rwkv
            for i in range(32):
                frame_hidden_state_2[:, :, i, :] = self.RWKV_block_0.rwkv(frame_hidden_state_1[:, :, i, :])

            video_hidden = torch.mean(frame_hidden_state_2, dim=1)

            video_tokens = self.RWKV_block_0.opt_proj_RWKV(video_hidden)
    ...
    def forward(self, samples):
        ...
        #subject
        video_embeds_subject, atts_video_subject = self.encode_video_rwkv_0(image_query_tokens)

        b_and_a_tokens = torch.cat([image_query_tokens, image_query_tokens], dim=0)
        #before_and_after
