...
# rwkv
from RWKV_5.qformer_lead_rwkv_init import rwkv_init
...
@registry.register_model("common_Qformer_lead_difference_RWKV_2o_3f")
class VideoBLIP2OPT_common_Qformer_lead_difference_RWKV_2o_3f(Blip2Base):
    """
    ...
    def __init__(
        ...
        #768-4096
        # self.RWKV_line = nn.Linear(
        #     768,self.q_former_hidden_size
        # )
        #4096--2560
        ...
        #self.NCT_subject = NCT_subject(NCT_config)
        ...
        self.RWKV_block = nn.Sequential(OrderedDict([
            ('rwkv', self.rwkv),
            #('rwkv_line',self.RWKV_line),
            ('opt_proj_RWKV', self.opt_proj_RWKV)
        ]))

        #self.avg_pool2d = nn.AvgPool2d(kernel_size=(1,2),stride=(1,2))
        self.avg_pool2d = nn.MaxPool2d(kernel_size=(1, 2), stride=(1, 2))
    ...
    def cosine_similarity(self, Q, K):
        ...
        return torch.matmul(Q_norm, K_norm.transpose(-1, -2))


    def cosine_cross_attention(self, Q, K, V):
        # Step 1: Compute similarity matrix
        S = self.cosine_similarity(Q, K)
    ...
    def common_subject(self,subject_feat):
        batch_size, frames, querys, hiden_dim = subject_feat.size()
        common_feat = torch.zeros(subject_feat.size())
        for i in range(frames - 1):
            common_feat[:,i,:,:] = self.cosine_cross_attention(Q=subject_feat[:,0,:,:], K=subject_feat[:,i + 1,:,:], V=subject_feat[:,i + 1,:,:])

        mean_value = common_feat[:,0:frames-1,:,:].mean(dim=1)
        common_feat[:,frames-1,:,:] = mean_value

        return common_feat

    def difference_subject(self,all_feat):

        # bef_difference_feat =before_feat - self.cosine_cross_attention(Q=before_feat, K=after_feat, V=after_feat)
        # #aft_difference_feat = after_feat - self.cosine_cross_attention(Q=after_feat, K=before_feat, V=before_feat)
        # return bef_difference_feat
        batch_size, frames, querys, hiden_dim = all_feat.size()
        difference_feat = torch.zeros(all_feat.size())
        for i in range(frames - 1):
            difference_feat[:, i, :, :] = all_feat[:, i + 1, :, :] - self.cosine_cross_attention(Q=all_feat[:, 0, :, :],
                                                                  K=all_feat[:, i + 1, :, :],
                                                                  V=all_feat[:, i + 1, :, :])

        mean_value = difference_feat[:, 0:frames - 1, :, :].mean(dim=1)
        difference_feat[:, frames - 1, :, :] = mean_value

        return difference_feat
    ...
    def encode_video_qformer(self, q_hidden_state, reference_points):
        with self.maybe_autocast():
            ...
            # 无position
            frame_hidden_state = q_hidden_state  # [12,12,32,768]
            #frame_hidden_state = self.cosine_cross_attention(Q=frame_hidden_state,K=frame_hidden_state,V=frame_hidden_state)
            # common_subject_feat = self.Qformer_block.NCT_subject(frame_hidden_state)
            # common_subject_feat = common_subject_feat.to(self.my_device)
            common_subject_feat = self.common_subject(frame_hidden_state)
            common_subject_feat = common_subject_feat.to(self.my_device)
            #frame_hidden_state = frame_hidden_state + common_subject_feat
            frame_hidden_state = torch.cat([frame_hidden_state, common_subject_feat], dim=2)

            #frame_hidden_state = self.cosine_cross_attention(Q=common_subject_feat,K=frame_hidden_state,V=frame_hidden_state)
            #common_subject_feat = torch.mean(common_subject_feat, dim=1)
            #frame_hidden_state = common_subject_feat
            #####################################################
            ...
            #video_query_tokens = video_query_tokens*common_subject_feat
            #print(video_query_tokens.shape)
            ...
            #video_hidden = common_subject_feat
            #########################################################################################
        ...
        return video_tokens, video_att_mask, video_query_output.last_hidden_state

    def encode_video_rwkv(self, q_hidden_state,subject,difference):
        with self.maybe_autocast():
            # add frame_pos embedding
            batch_size, time_length, _, _ = q_hidden_state.size()

            #frame_hidden_state = q_hidden_state  # [24,12,64,768]
            ...
            # rwkv
            repeated_subject = subject.unsqueeze(1).repeat(1, 12, 1, 1)#(12,12,32,768)
            repeated_2subject = torch.cat([repeated_subject, repeated_subject], dim=0)#(24,12,32,768)
            #repeated_3subject = torch.cat([repeated_2subject,repeated_2subject],dim=1)#(24,24,32,768)
            #subject_difference = self.cosine_cross_attention(Q=difference,K=repeated_2subject,V=repeated_2subject)
            #subject_difference = repeated_2subject + difference
            ###########################################################################################

            # feat_subject_difference = torch.cat([frame_hidden_state_1, difference], dim=3)  # (24,24,32,1536)
            # feat_subject_difference = self.avg_pool2d(feat_subject_difference)
            #feat_subject_difference = self.leaky_relu(feat_subject_difference)

            ###########################################################################################

            ###########################################################################################
            difference = torch.cat([difference,difference],dim=0)
            lead_subject_difference = torch.cat([repeated_2subject,difference],dim=3)#(24,24,32,1536)
            lead_subject_difference = self.avg_pool2d(lead_subject_difference)
            lead_subject_difference = self.leaky_relu(lead_subject_difference)
            ###########################################################################################
            ...
            for i in range(32):
                frame_hidden_state_2[:, :, i, :] = self.RWKV_block.rwkv(frame_hidden_state_1[:, :, i, :],
                                                                        lead_subject_difference[:, :, i, :],
                                                                        )
            # frame_hidden_state_2 = self.difference_subject(frame_hidden_state_2)
            # frame_hidden_state_2 = frame_hidden_state_2.to(self.my_device)

            #difference = self.RWKV_block.rwkv_line(difference)

            # difference = difference.repeat(1,1,1,5)
            # padding_size = 4096 - 3840
            # difference = torch.nn.functional.pad(difference,(0,padding_size))
            #
            # frame_hidden_state_2 = torch.cat([frame_hidden_state_2,difference],dim=2)
            #
            video_hidden = torch.mean(frame_hidden_state_2, dim=1)
            #print(video_hidden.shape)
            #video_hidden = self.RWKV_block.rwkv_line(video_hidden)
            # difference_mean = torch.mean(difference, dim=1)
            # #
            # difference_mean = self.RWKV_block.rwkv_line(difference_mean)
            # video_hidden = self.cosine_cross_attention(Q=difference_mean,K=video_hidden,V=video_hidden)
    ...
    def forward(self, samples):
        ...
        # image_query_tokens = image_query_tokens.to('cuda:1')
        # reference_points = reference_points.to('cuda:1')
        # before_tokens = before_tokens.to('cuda:1')
        # after_tokens = after_tokens.to('cuda:1')

        #subject
        video_embeds_subject, atts_video_subject, subject_video_tokens = self.encode_video_qformer(image_query_tokens, reference_points)
        # print(video_embeds_subject.shape)
        # st
        b_and_a_tokens = torch.cat([before_tokens, after_tokens], dim=0)  # (24,12,32,768)
        #before_difference_feat = self.difference_subject(before_tokens, after_tokens)  # (12,12,32,768)
        b_and_a_tokens_difference = self.difference_subject(image_query_tokens)
        b_and_a_tokens_difference = b_and_a_tokens_difference.to(self.my_device)
        b_and_a_tokens_difference = self.leaky_relu(b_and_a_tokens_difference)
        #b_and_a_tokens = self.cosine_cross_attention(Q=b_and_a_tokens_difference,K=b_and_a_tokens,V=b_and_a_tokens)
        #b_and_a_tokens = torch.cat([b_and_a_tokens,b_and_a_tokens_difference],dim=1)
        #before_and_after
        video_embeds_b_and_a, atts_video_b_and_a= self.encode_video_rwkv(b_and_a_tokens,subject_video_tokens,b_and_a_tokens_difference)
        #_, _ = self.encode_video_rwkv(b_and_a_tokens, subject_video_tokens)
        #after
        #video_embeds_after, atts_video_after = self.encode_video_rwkv(after_tokens)


        #atts_video_b_and_a = torch.cat([atts_video_before, atts_video_after], dim=0)
        # loss_subject
        ########################################################################
    ...
    def generate(
        ...
        with self.maybe_autocast():
            ...
            b_and_a_tokens_difference = b_and_a_tokens_difference.to(self.my_device)
            b_and_a_tokens_difference = self.leaky_relu(b_and_a_tokens_difference)
            # b_and_a_tokens = self.cosine_cross_attention(Q=b_and_a_tokens_difference, K=b_and_a_tokens,
            #                                              V=b_and_a_tokens)
            ...
            # #before
            # video_embeds_before, atts_video_before = self.encode_video_rwkv(before_tokens)
            # #after
            # video_embeds_after, atts_video_after = self.encode_video_rwkv(after_tokens)
            #
            # video_embeds_b_and_a = torch.cat([video_embeds_before, video_embeds_after], dim=0)
            # atts_video_b_and_a = torch.cat([atts_video_before, atts_video_after], dim=0)
