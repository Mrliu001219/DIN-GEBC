...
@registry.register_model("common_Qformer_lead_RWKV_2o_3f")
class VideoBLIP2OPT_common_Qformer_lead_RWKV_2o_3f(Blip2Base):
    """
    ...
    def encode_video_qformer(self, q_hidden_state, reference_points):
        with self.maybe_autocast():
            ...
            #frame_hidden_state = self.leaky_relu(frame_hidden_state)
    ...
    def encode_video_rwkv(self, q_hidden_state,subject):
        with self.maybe_autocast():
            # add frame_pos embedding
            batch_size, time_length, _, _ = q_hidden_state.size()
            ...
            # rwkv
            repeated_subject = subject.unsqueeze(1).repeat(1, 12, 1, 1)
            repeated_2subject = torch.cat([repeated_subject, repeated_subject], dim=0)
            # rwkv
            for i in range(32):
    ...
    def forward(self, samples):
        ...
        #before_and_after
        video_embeds_b_and_a, atts_video_b_and_a= self.encode_video_rwkv(b_and_a_tokens,subject_video_tokens)
