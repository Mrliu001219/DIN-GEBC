...
@registry.register_model("Qformer_lead_RWKV_2o_3f")
class VideoBLIP2OPT_Qformer_lead_RWKV_2o_3f(Blip2Base):
    """
    ...
    def __init__(
        ...
        # self.subject_lead_rwkv_line = nn.Linear(
        #     self.opt_model.config.hidden_size, self.subject_q_former_hidden_size
        # )
        # video_qformer
        ...
        self.Qformer_block = nn.Sequential(OrderedDict([
            ...
            #('subject_lead_rwkv_line', self.subject_lead_rwkv_line)
        ]))
    ...
    def encode_video_qformer(self, q_hidden_state, reference_points):
        with self.maybe_autocast():
            ...
            #subject_video_tokens = self.Qformer_block.subject_lead_rwkv_line(video_tokens)
    ...
    def encode_video_rwkv(self, q_hidden_state,subject):
        with self.maybe_autocast():
            ...
            # rwkv
            for i in range(32):
                frame_hidden_state_2[:, :, i, :] = self.RWKV_block.rwkv(frame_hidden_state_1[:, :, i, :],repeated_2subject[:, :, i, :])
    ...
    def forward(self, samples):
        ...
        # repeated_subject = video_token.unsqueeze(1).repeat(1, 12, 1, 1)
        # print(repeated_subject.shape)
        # st
        #before_and_after
