...
def split_pred(pred):
    ...
    for pred_item in pred:
        video_id = pred_item['video_id']
        caption = pred_item['caption']
        pred_dict[video_id] = [caption]
    return pred_dict
...
def split_gt(gt):
    ...
    for video_id, capton in gt.items():
        gt_dict[video_id] = capton

    return gt_dict


def msvd_captioning_eval(pred_file_path, gt_file_path):
    ...
    res_pred_scores = evaluate_on_caption(pred_dict, gt_dict)

    #all_scores = [res_pred_scores]
    # all_scores = [res_pred_sub]
    # mean_scores = {'METEOR': 0, 'ROUGE_L': 0, 'CIDEr': 0, 'Bleu_4': 0}
    ...
    overall_score = 0
    n = 0
    for key, val in res_pred_scores.items():
        overall_score += val
        n += 1
    mean_scores['mean_score'] = overall_score * 100 / n

    subject_score = {'msvd_' + metric: val * 100 for metric, val in res_pred_scores.items()}
    ...
    def setBoundaryToEvalBoundaries(self, scores, b_ids, method):
        for b_id, score in zip(b_ids, scores):
            if not b_id in self.BoundariesToEval:
                ...
                self.BoundariesToEval[b_id]["video_id"] = b_id
