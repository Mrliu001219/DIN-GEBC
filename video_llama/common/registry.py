...
class Registry:
    ...
    def register_model(cls, name):
        ...
        def wrap(model_cls):#model_cls:类
            from video_llama.models import BaseModel
            #issubclass是一个内置函数，用于检查一个类是否是另一个类的子类
            assert issubclass(
                model_cls, BaseModel
            ), "All models must inherit BaseModel class"
            if name in cls.mapping["model_name_mapping"]:#检查此类是否已经注册
                raise KeyError(
                    "Name '{}' already registered for {}.".format(
                        name, cls.mapping["model_name_mapping"][name]
                    )
                )
            cls.mapping["model_name_mapping"][name] = model_cls#未注册的话注册此类
            return model_cls
    ...
    def list_runners(cls):
        return sorted(cls.mapping["runner_name_mapping"].keys())#排序
