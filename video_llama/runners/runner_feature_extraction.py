...
class RunnerFeatureExtraction:
    ...
    def data_loader(self):
        if not hasattr(self, '_dataloader'):
            ...
            self._dataloader = DataLoader(  # torch官方DataLoader
                self.dataset,
    ...
    # 运行，调用tasks里的video_feature_extraction.py中的feature_extraction
    def start_extract(self):  # 提取保存特征
        self.task.feature_extraction(self.model, self.data_loader, self.save_dir)  # 输入初始化好的模型，dataloader
