...
def load_video(video_path, n_frms=MAX_INT, height=-1, width=-1, sampling="uniform", return_msg = False):
    decord.bridge.set_bridge("torch")
    #创建一个视频读取器对象vr
    vr = VideoReader(uri=video_path, height=height, width=width)#视频输出高度和宽度，如果为-1则保持不变
    ...
    if sampling == "uniform":
                            #（开始值、结束值、步长），不包括结束值
        indices = np.arange(start, end, vlen / n_frms).astype(int).tolist()#将数组转换成列表
    ...
    # get_batch -> T, H, W, C
    temp_frms = vr.get_batch(indices)#获取批量视频帧，indices：整数列表
    # print(type(temp_frms))
    #将numpy数组转换为tensor数据
    tensor_frms = torch.from_numpy(temp_frms) if type(temp_frms) is not torch.Tensor else temp_frms
    frms = tensor_frms.permute(3, 0, 1, 2).float()  # (C, T, H, W)，交换维度
    #false，条件成立
    if not return_msg:
        return frms
    #获取平均帧率，返回浮点数
    fps = float(vr.get_avg_fps())
    #将一个列表中的元素以逗号分隔并连接成一个字符串
...
class AlproVideoEvalProcessor(AlproVideoBaseProcessor):
    def __init__(self, image_size=256, mean=None, std=None, n_frms=MAX_INT):
        ...
        # Input video size is (C, T, H, W)输入tensor格式，变换形状
        self.transform = transforms.Compose(
            [
