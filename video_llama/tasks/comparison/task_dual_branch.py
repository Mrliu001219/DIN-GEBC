...
@registry.register_task("task_dual_branch")
class Task_Dual_Branch(BaseTask):
...
#######################################################################################################
    ...
    def _train_inner_loop(
        ...
        metric_logger.add_meter("lr_rwkv_0", SmoothedValue(window_size=1, fmt="{value:.6f}"))
        metric_logger.add_meter("lr_rwkv", SmoothedValue(window_size=1, fmt="{value:.6f}"))
        metric_logger.add_meter("loss_rwkv_0", SmoothedValue(window_size=1, fmt="{value:.4f}"))
        ...
        for i in metric_logger.log_every(range(iters_per_epoch), log_freq, header):
            ...
            with torch.cuda.amp.autocast(enabled=use_amp):
                loss_rwkv_0, loss_rwkv = self.train_step(model=model, samples=samples)
                #loss_qformer= self.train_step(model=model, samples=samples)
            optimizer_rwkv_0, optimizer_rwkv = optimizer
            #  hight-1.25   puar_hight:1.20,      comm_full:1.00    puar-full:1.10   full:1.20
            if self.avg_rwkv_0_loss > 1.50:
                optimizer_rwkv_0.zero_grad()
                loss_rwkv_0.backward()
                optimizer_rwkv_0.step()
...
################################################################################
            ...
            metric_logger.update(loss_rwkv_0=loss_rwkv_0.item())
            metric_logger.update(loss_rwkv=loss_rwkv.item())
            metric_logger.update(lr_rwkv_0=optimizer_rwkv_0.param_groups[0]["lr"])
        ...
        self.avg_rwkv_0_loss = metric_logger.meters['loss_rwkv_0'].global_avg
        print(self.avg_rwkv_0_loss)
