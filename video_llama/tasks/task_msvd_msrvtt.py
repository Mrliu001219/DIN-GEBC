...
from video_llama.evaluation.msvd_evaluation import msvd_captioning_eval
...
@registry.register_task("task_msvd_msrvtt")
class Task_MSVD_MSRVTT(BaseTask):
...
#############################################################################################################
        ...
        video_ids = samples["video_id"]

        for caption, video_id in zip(captions, video_ids):
            results.append({"caption": caption, "video_id": video_id})
...
#######################################################################################################
    ...
    def _report_metrics(self, eval_result_file, split_name):
        ...
        gt_file = self.cfg.datasets_cfg.msvd_msrvtt_builder.build_info.annotations.get(split_name).annotation_path
        #gt_file = self.cfg.datasets_cfg.gebc_builder_Qformer_lead_RWKV.build_info.annotations.get(split_name).annotation_path

        scores = msvd_captioning_eval(eval_result_file, gt_file)
    ...
    def _train_inner_loop(
        ...
        for i in metric_logger.log_every(range(iters_per_epoch), log_freq, header):
            ...
            optimizer_msvd = optimizer

            optimizer_msvd.zero_grad()
            loss.backward()
            optimizer_msvd.step()


            metric_logger.update(loss=loss.item())
            metric_logger.update(lr=optimizer_msvd.param_groups[0]["lr"])
